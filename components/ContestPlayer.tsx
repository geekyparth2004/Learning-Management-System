...
export default function ContestPlayer({ contest, problems, endTime, onLeave }: ContestPlayerProps) {
    ...
    useEffect(() => {
        ...
        setUserCodes(prev => {
            const next = { ...prev };
            problems.forEach(p => {
                if (next[p.id] === undefined) next[p.id] = codes[p.id];
            });
            return next;
        });
    }, [problems, language]);
    ...
    useEffect(() => {
        ...
        if ((!userCodes[activeProblem.id] && userCodes[activeProblem.id] !== "") && activeProblem.type !== "WEB_DEV") {
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <div className="w-1/2 flex flex-col bg-[#111111]">
                    ...
                    <div className="flex-1 relative">
                        {activeTab === "editor" && (
                            <textarea
                                className="w-full h-full bg-[#1e1e1e] text-white p-4 font-mono outline-none resize-none"
                                value={userCodes[activeProblem.id] as string || ""}
