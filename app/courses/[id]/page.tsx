    ...
    const savePracticeCode = async () => {
        ...
        if (practiceType === "web") {
            try {
                ...
                if (res.ok) {
                    alert("Web Dev Code saved to GitHub!");
                } else {
                    const data = await res.json();
                    alert(data.error || "Failed to save Web Dev code");
                }
            } catch (e) {
                console.error(e);
                alert("Error saving Web Dev code");
            }
            return;
        }

        // DSA Practice Save (Existing)
        try {
                                                    ...
                                                    <video
                                                        ...
                                                        onEnded={(e) => {
                                                            ...
                                                            if (video.duration && video.currentTime >= video.duration - 1) {
                                                                if (activeItem) {
                                                                    completeItem(activeItem.id, Math.floor(accumulatedTime), true, true);
                                                                    setAccumulatedTime(0);
                                                                }
                                                            }
                                                        }}
                                                        onError={(e) => {
                                                            ...
                                                            // Optional: Toast or UI indication could go here
                                                        }}
                                                    />
                                                );
                                            }
